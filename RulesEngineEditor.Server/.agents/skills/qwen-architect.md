...
2. Verify the target table exists in the `Tables` array — exact match `"[dbo].[{TableName}]"` with `"ObjectType": 0`
   ...
   - If found in Models: STATE = TABLE_EXISTS_BUT_NOT_WHITELISTED (suggest Power Tools Refresh)
   - If NOT found in Models either: STATE = TABLE_DOES_NOT_EXIST (full creation needed)
...
4. Verify ANY custom stored procedures specified in feature tasks (e.g., `sp_ExecuteWorkflow`) exist — separate path from CRUD 5-pack
5. If any object is MISSING and not AspNet* → execute the **Agentic Correction Routine**:
...
> **Note**: Technology stack details are maintained in `deepseek-architect.md` as the source of truth. This Qwen file mirrors the same structure with model-specific assignments.
